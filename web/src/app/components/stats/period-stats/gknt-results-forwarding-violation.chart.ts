...
export class GkntResultsForwardingViolationChart implements OnInit {
  ...
  private render(stats: StatsDto[]) {
    ...
    this.chart = Chart.chart('Контроль времени направления результатов')
      ...
      .exporting(Chart.exporting()
        .filename('Контроль времени направления результатов')
        .size(625, 350))
      .tooltip(Chart.tooltip().shared())
      .series(Chart.columnOptions('Процент нарушений сроков')
        .data(stats.map(stats => {
          return {
            y: TermsStatsDto.violationsPercent(stats.gkntResultsForwardingTermsStats),
            all: stats.gkntResultsForwardingTermsStats.totalCount,
            count: stats.gkntResultsForwardingTermsStats.violations,
          }
        }))
      ...
      .series(Chart.lineOptions('Среднее время рассмотрения')
        .data(stats.map(stats => stats.gkntResultsForwardingTermsStats.averageTime))
        .color(blueClr)
        .yAxis(1)
        .tooltip(Chart.tooltip()
