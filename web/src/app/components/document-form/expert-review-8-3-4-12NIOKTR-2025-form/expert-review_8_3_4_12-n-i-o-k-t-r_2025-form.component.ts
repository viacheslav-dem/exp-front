...
export class ExpertReview_8_3_4_12NIOKTR_2025FormComponent extends ExpertReviewForm<ExpertReview_8_3_4_12NIOKTR_2025FormContent> {
  ...
  private validationLengthCommentsOnConclusionRelusAndResult() {
    ...
    if (this._form.targetAccordanceText.length < 30 || this._form.targetAccordanceText.length > 5000) {
      throw "В пункте 'Соответствие заявленному программному инструменту реализации.' комментарий должен быть не менее 30 символов и не более 5000 символов."
    }
  ...
  private validationLengthCommentsOnConclusionAnalysisAndEvaluation() {
    ...
    if (this._form.scientificResearchText.length < 30 || this._form.scientificResearchText.length > 5000) {
      throw "В пункте 'Вид научного исследования.' комментарий должен быть не менее 30 символов и не более 5000 символов."
    }
    ...
    if (this.showTarget8_3() && this._form.targetText.length < 30 || this._form.targetText.length > 5000) {
      throw "В пункте 'Оценка целевых показателей проекта.' комментарий должен быть не менее 30 символов и не более 5000 символов."
    }
    if (this.showTarget8_4() && this._form.taskListsText.length < 30 || this._form.taskListsText.length > 5000) {
    ...
    if (this._form.technologicalOrderText.length < 30 || this._form.technologicalOrderText.length > 5000) {
      throw "В пункте 'Уровень технологического уклада научно-технической продукции.'комментарий должен быть не менее 30 символов и не более 5000 символов."
    }
    if ((this.showTarget8_3() || this.showTarget8_4()) && (this._form.commerceText.length < 30 || this._form.commerceText.length > 5000)) {
    ...
    if (this._form.competitivenessText.length < 30 || this._form.competitivenessText.length > 5000) {
      throw "В пункте 'Обоснование конкурентоспособности разработки.' комментарий должен быть не менее 30 символов и не более 5000 символов."
    }
    if (this._form.analogText.length < 30 || this._form.analogText.length > 5000) {
      throw "В пункте 'Направленность объекта государственной экспертизы.' комментарий должен быть не менее 30 символов и не более 5000 символов."
    }
    if (this._form.analogParamsText.length < 30 || this._form.analogParamsText.length > 5000) {
    ...
    if (this._form.needsText.length < 30 || this._form.needsText.length > 5000) {
      ...
      "Евразийского экономического союза и дальнего зарубежья.' комментарий должен быть не менее 30 символов и не более 5000 символов."
    }
    if (this._form.risksText.length < 30 || this._form.risksText.length > 5000) {
      throw "В пункте 'Риски реализации проекта.' комментарий должен быть не менее 30 символов и не более 5000."
    }
