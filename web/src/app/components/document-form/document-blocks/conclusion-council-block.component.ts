...
@Component({
    ...
    <div class="form-sub-group">
      ...
      <textarea [ngModel]="form()?.conclusionText" (ngModelChange)="emitPatch({ conclusionText: $event })" name="conclusionText" required rows="3" class="form-control"
...
export class ConclusionCouncilBlockComponent {
  ...
  readonly form = input<ConclusionCouncilBlockForm>(undefined);

  readonly group = input<LifecycleGroupDto>(undefined);

  readonly onConditionsChanged = output<boolean>();
  readonly formPatch = output<Partial<ConclusionCouncilBlockForm>>();

  emitPatch(patch: Partial<ConclusionCouncilBlockForm>) {
    this.formPatch.emit(patch);
    this.onConditionsChanged.emit(true);
  }
}

type ConclusionCouncilBlockForm = {
